...
mut_list <- coloE_samples %>%
  select(6:68)
coloE_samples <- coloE_samples %>%
  select(3, 6:68) 
coloE_size <- dim(coloE_samples)[1]

mut <- as.numeric(colo_samples[1,])
nonzero_sig <- (which(mut %nin% c(0, NA)))-1
list <- colnames(mut_list)
sig_list <- list[nonzero_sig]
sig_list <- str_remove(sig_list, ".Mutations")
mut_list <- mut[mut %nin% c(0, NA)]
...
#ignore
view(colo_samples)
for (i in generate_sample_prof(c())){
  sample_prof <- generate_sample_prof(c("SBS1","SBS5","SBS44"),c(31717,46279,46366))
}
```
