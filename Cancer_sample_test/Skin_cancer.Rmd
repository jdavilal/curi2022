...
Skin_cancer_samples <- COSMICsample_ds %>%
  filter(Cancer.Types == "Skin-Melanoma") %>%
  filter(Scale == "exome")

Skin_cancer_samples_withnames <- Skin_cancer_samples %>%
  select(4,6:64) 
Skin_cancer_samples <- Skin_cancer_samples %>%
  select(6:64) 
Skin_cancer_samples_size <- dim(Skin_cancer_samples)[1]

sample_names_list <- Skin_cancer_samples_withnames %>%
    select(1)
...
Making the final Dataframe for skin melanoma cancer 
...
for (i in 1:Skin_cancer_samples_size){
  
  mut <- as.numeric(Skin_cancer_samples[i,])
  nonzero_sig <- (which(mut %nin% c(0, NA)))
  list <- colnames(Skin_cancer_samples)
  ...
  while(neg_count != 0){
    ...
    model <- dissolvo(sample_prof, sigs = s_list, tissue_type = "SKCM", scale = "exome", quantiles = c("5%","95%"), mb_size = 50)
...
eval_df_2 <- cbind(sample_list, sig_list, expected_list, fitsig_list, actual_list)
eval_df_2 <- as_tibble(eval_df_2)
eval_df_2 <- eval_df_2 %>%
  mutate(
...
```{r}
write.csv(eval_df_2, "Data/skin_cancer_evaluation_ds.csv", row.names = FALSE)
```

Plot the Actual vs. Expected
```{r}
eval_df_2 <- read_csv("~/Desktop/curi2022/Data/skin_cancer_evaluation_ds.csv")
ggplot(data = eval_df_2, mapping = aes(x = expected_list, y = actual_list))+
   geom_point(alpha=0.3)+
   geom_smooth(method = "lm")+
  facet_wrap(~sig_list)

cor(eval_df_2$expected_list, eval_df_2$actual_list)

cor(eval_df_2$actual_list, eval_df_2$fitsig_list)
```
...
for (i in 1:Skin_Melanoma_samples_size){
  
  mut <- as.numeric(Skin_Melanoma_samples[i,])
  nonzero_sig <- (which(mut %nin% c(0, NA)))
  list <- colnames(Skin_Melanoma_samples)
  s_list <- list[nonzero_sig]
  s_list <- str_remove(s_list, ".Mutations")
  m_list <- mut[mut %nin% c(0, NA)]
  ...
  method_test <- estimate_coeff(sample_prof, sigs = s_list, tissue_type = "SKCM", scale = "exome", quantiles = c("5%", "95%"), mb_size = 50)
...
Skin_cancer_sample_names <- unique(evaluation_ds$sample_list)
mse_df_2 <- cbind(Skin_cancer_sample_names, mse_list_EF, mse_list_EA)
mse_df_2 <- as_tibble(mse_df_2)
mse_df_2 <- mse_df_2 %>%
  mutate(
...
```{r}
write.csv(mse_df_2, "Data/mse_ds_Skin_cancer.csv", row.names = FALSE)
```

Plotting MSE between Expected vs. fit_to_signature vs. Actual
```{r}
mse_df_2 <- read_csv("~/Desktop/curi2022/Data/mse_ds_Skin_cancer.csv")
```
```{r}
mean(mse_df_2$mse_list_EF)
mean(mse_df_2$mse_list_EA)

histogram(mse_df_2$mse_list_EF)
histogram(mse_df_2$mse_list_EA)

mse_df_2 %>%
  ggplot(aes(x = seq(1, dim(mse_df_2)[1]), y = mse_list_EA)) +
    geom_point() +
    labs(x = "samples", y = "MSE", title = "MSE between Expected and Actual value for Skin Melanoma cancer") 

mse_df_2 %>%
  ggplot(aes(x = seq(1, dim(mse_df_2)[1]), y = mse_list_EF)) +
    geom_point() +
    labs(x = "samples", y = "MSE", title = "MSE between Expected and fit_to_signature value for Skin Melanoma cancer") 
```