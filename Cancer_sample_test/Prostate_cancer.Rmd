...
#Testing for Prostate cancer, exome data
```{r}
...
Prostate_cancer_samples <- COSMICsample_ds %>%
  filter(Cancer.Types == "Prost-AdenoCA") %>%
  filter(Scale == "exome")

Prostate_cancer_samples_withnames <- Prostate_cancer_samples %>%
  select(4,6:64) 
Prostate_cancer_samples <- Prostate_cancer_samples %>%
  select(6:64) 
Prostate_cancer_samples_size <- dim(Prostate_cancer_samples)[1]

sample_names_list <- Prostate_cancer_samples_withnames %>%
    select(1)
sample_names_list <- as.matrix(sample_names_list)
```


Making the final Dataframe for Prostate cancer 
...
for (i in 1:Prostate_cancer_samples_size ){
  
  mut <- as.numeric(Prostate_cancer_samples[i,])
  nonzero_sig <- (which(mut %nin% c(0, NA)))
  list <- colnames(Prostate_cancer_samples)
  ...
  while(neg_count != 0){
    ...
    model <- dissolvo(sample_prof, sigs = s_list, tissue_type = "PRAD", scale = "exome", quantiles = c("5%","95%"), mb_size = 50)
...
eval_df_16 <- cbind(sample_list, sig_list, expected_list, fitsig_list, actual_list)
eval_df_16 <- as_tibble(eval_df_16)
eval_df_16 <- eval_df_16 %>%
  mutate(
...
```{r}
write.csv(eval_df_16, "Data/Prostate_cancer_evaluation_ds.csv", row.names = FALSE)
```

Plot the Actual vs. Expected
```{r}
eval_df_16 <- read_csv("~/Desktop/curi2022/Data/Prostate_cancer_evaluation_ds.csv")
ggplot(data = eval_df_16, mapping = aes(x = expected_list, y = actual_list))+
   geom_point(alpha=0.3)+
   geom_smooth(method = "lm")+
  facet_wrap(~sig_list)

cor(eval_df_16$expected_list, eval_df_16$actual_list)

cor(eval_df_16$actual_list, eval_df_16$fitsig_list)
```