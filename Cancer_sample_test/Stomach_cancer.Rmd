...
#Testing for Stomach cancer, exome data
```{r}
...
Stomach_cancer_samples <- COSMICsample_ds %>%
  filter(Cancer.Types == "Stomach-AdenoCA") %>%
  filter(Scale == "exome")

Stomach_cancer_samples_withnames <- Stomach_cancer_samples %>%
  select(4,6:64) 
Stomach_cancer_samples <- Stomach_cancer_samples %>%
  select(6:64) 
Stomach_cancer_samples_size <- dim(Stomach_cancer_samples)[1]

sample_names_list <- Stomach_cancer_samples_withnames %>%
    select(1)
sample_names_list <- as.matrix(sample_names_list)
```


Making the final Dataframe for Stomach cancer 
...
for (i in 1:300){
  ...
  mut <- as.numeric(Stomach_cancer_samples[i,])
  nonzero_sig <- (which(mut %nin% c(0, NA)))
  list <- colnames(Stomach_cancer_samples)
  ...
  while(neg_count != 0){
    ...
    model <- dissolvo(sample_prof, sigs = s_list, tissue_type = "STAD", scale = "exome", quantiles = c("5%","95%"), mb_size = 50)
...
eval_df_15 <- cbind(sample_list, sig_list, expected_list, fitsig_list, actual_list)
eval_df_15 <- as_tibble(eval_df_15)
eval_df_15 <- eval_df_15 %>%
  mutate(
...
```{r}
write.csv(eval_df_15, "Data/Stomach_cancer_evaluation_ds.csv", row.names = FALSE)
```

Plot the Actual vs. Expected
```{r}
eval_df_15 <- read_csv("~/Desktop/curi2022/Data/Stomach_cancer_evaluation_ds.csv")
ggplot(data = eval_df_15, mapping = aes(x = expected_list, y = actual_list))+
   geom_point(alpha=0.3)+
   geom_smooth(method = "lm")+
  facet_wrap(~sig_list)

cor(eval_df_15$expected_list, eval_df_15$actual_list)

cor(eval_df_15$actual_list, eval_df_15$fitsig_list)
```