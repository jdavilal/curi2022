...
lm_test(sample_profile,c("SBS10a","SBS6"), c(0.1, 1, 0.1, 1))

#3 sigs
sample_profile <- generate_sample_prof(c("SBS1","SBS10b","SBS6"),c(200,300,500))
plot_sample_prof(sample_profile)
lm_test(sample_profile,c("SBS1","SBS10b","SBS6"), c(0.1, 1, 0.1, 1, 0.1, 1))

#4 sigs
sample_profile <- generate_sample_prof(c("SBS10a","SBS10b","SBS10c","SBS6"),c(300,100,200,400))
plot_sample_prof(sample_profile)
lm_test(sample_profile,c("SBS10a","SBS10b","SBS10c","SBS6"), c(0.1, 1, 0.1, 1, 0.1, 1, 0.1, 1))

#5 sigs
sample_profile <- generate_sample_prof(c("SBS10a","SBS10b","SBS10c","SBS10d", "SBS6"),c(200,300,100,300,100))
plot_sample_prof(sample_profile)
lm_test(sample_profile,c("SBS10a","SBS10b","SBS10c","SBS10d", "SBS6"), c(0.1, 1, 0.1, 1, 0.1, 1, 0.1, 1, 0.1, 1))
```
...
constr1 <- constraints_ds %>%
  filter(Tissue == "BRCA", Signature == sigs[1] | Signature == sigs[2], Scale == "exome", Quantile == quantiles[1] | Quantile == quantiles[2]) 
...
```{r}
test1 <- constraints_ds %>%
  filter(Tissue == "BRCA", Signature == "SBS1" | Signature == "SBS10a", Scale == "exome", Quantile == "5%" | Quantile == "95%")

test2 <- c(test1$Value)

test2 <- constraints_ds %>%
  filter(Tissue == "BRCA", Signature == "SBS1" | Signature == "SBS10a", Scale == "genome")
```
