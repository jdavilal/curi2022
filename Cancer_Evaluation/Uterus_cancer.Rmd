...
#Testing for Uterus cancer, exome data
```{r}
...
Uterus_cancer_samples <- COSMICsample_ds %>%
  filter(Cancer.Types == "Uterus-AdenoCA") %>%
  filter(Scale == "exome")

Uterus_cancer_samples_withnames <- Uterus_cancer_samples %>%
  select(4,6:64) 
Uterus_cancer_samples <- Uterus_cancer_samples %>%
  select(6:64) 
Uterus_cancer_samples_size <- dim(Uterus_cancer_samples)[1]

sample_names_list <- Uterus_cancer_samples_withnames %>%
    select(1)
sample_names_list <- as.matrix(sample_names_list)
```


Making the final Dataframe for Uterus cancer 
...
for (i in 1:Uterus_cancer_samples_size){
  
  mut <- as.numeric(Uterus_cancer_samples[i,])
  nonzero_sig <- (which(mut %nin% c(0, NA)))
  list <- colnames(Uterus_cancer_samples)
  ...
  while(neg_count != 0){
    ...
    model <- dissolvo(sample_prof, sigs = s_list, tissue_type = "UCS", scale = "exome", quantiles = c("5%","95%"), mb_size = 50)
...
eval_df_6 <- cbind(sample_list, sig_list, expected_list, fitsig_list, actual_list)
eval_df_6 <- as_tibble(eval_df_6)
eval_df_6 <- eval_df_6 %>%
  mutate(
...
```{r}
write.csv(eval_df_6, "Data/Uterus_cancer_evaluation_ds.csv", row.names = FALSE)
```

Plot the Actual vs. Expected
```{r}
eval_df_6 <- read_csv("~/Desktop/curi2022/Data/Uterus_cancer_evaluation_ds.csv")
ggplot(data = eval_df_6, mapping = aes(x = expected_list, y = actual_list))+
   geom_point(alpha=0.3)+
   geom_smooth(method = "lm")+
  facet_wrap(~sig_list)

cor(eval_df_6$expected_list, eval_df_6$actual_list)

cor(eval_df_6$fitsig_list, eval_df_6$actual_list)
...
for (i in 1:Uterus_cancer_samples_size){
  ...
  if(length(s_list) == 1){
    cosmic_6sig <- as.matrix(cosmic.signatures[,s_list])
    colnames(cosmic_6sig) <- s_list
    
    fit_test <- fit_to_signatures_strict(sample_prof, cosmic_6sig, max_delta = 0.002)
...
Uterus_cancer_sample_names <- unique(evaluation_ds$sample_list)
mse_df_6 <- cbind(Uterus_cancer_sample_names, mse_list_EF, mse_list_EA)
mse_df_6 <- as_tibble(mse_df_6)
mse_df_6 <- mse_df_6 %>%
  mutate(
...
```{r}
write.csv(mse_df_6, "Data/mse_ds_Uterus_cancer.csv", row.names = FALSE)
```

Plotting MSE between Expected vs. fit_to_signature vs. Actual
```{r}
mse_df_6 <- read_csv("~/Desktop/curi2022/Data/mse_ds_Uterus_cancer.csv")
```
```{r}
mean(mse_df_6$mse_list_EF)
mean(mse_df_6$mse_list_EA)

histogram(mse_df_6$mse_list_EF)
histogram(mse_df_6$mse_list_EA)

mse_df_6 %>%
  ggplot(aes(x = seq(1, dim(mse_df_6)[1]), y = mse_list_EA)) +
    geom_point() +
    labs(x = "samples", y = "MSE", title = "MSE between Expected and Actual value for Uterus cancer") 

mse_df_6 %>%
  ggplot(aes(x = seq(1, dim(mse_df_6)[1]), y = mse_list_EF)) +
    geom_point() +
    labs(x = "samples", y = "MSE", title = "MSE between Expected and fit_to_signature value for Uterus cancer") 
```
