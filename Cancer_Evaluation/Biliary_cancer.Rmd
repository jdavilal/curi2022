...
#Testing for Biliary cancer, exome data
```{r}
...
Biliary_cancer_samples <- COSMICsample_ds %>%
  filter(Cancer.Types == "Biliary-AdenoCA") %>%
  filter(Scale == "exome")

Biliary_cancer_samples_withnames <- Biliary_cancer_samples %>%
  select(4,6:64) 
Biliary_cancer_samples <- Biliary_cancer_samples %>%
  select(6:64) 
Biliary_cancer_samples_size <- dim(Biliary_cancer_samples)[1]

sample_names_list <- Biliary_cancer_samples_withnames %>%
    select(1)
sample_names_list <- as.matrix(sample_names_list)
```


Making the final Dataframe for Biliary cancer 
...
for (i in 1:Biliary_cancer_samples_size){
  
  mut <- as.numeric(Biliary_cancer_samples[i,])
  nonzero_sig <- (which(mut %nin% c(0, NA)))
  list <- colnames(Biliary_cancer_samples)
  ...
  method_test <- estimate_coeff(sample_prof, sigs = s_list, tissue_type = "CHOL", scale = "exome", quantiles = c("5%", "95%"), mb_size = 50)
...
eval_df_8 <- cbind(sample_list, sig_list, expected_list, fitsig_list, actual_list)
eval_df_8 <- as_tibble(eval_df_8)
eval_df_8 <- eval_df_8 %>%
  mutate(
...
eval_df_8 <- cbind(sample_names, sig_list, expected_list, fitsig_list, actual_list)
eval_df_8 <- as_tibble(eval_df_8)
``` 

```{r}
write.csv(eval_df_8, "Data/Biliary_cancer_evaluation_ds.csv", row.names = FALSE)
```

Plot the Actual vs. Expected
```{r}
eval_df_8 <- read_csv("~/Desktop/curi2022/Data/Biliary_cancer_evaluation_ds.csv")
ggplot(data = eval_df_8, mapping = aes(x = expected_list, y = actual_list))+
   geom_point(alpha=0.3)+
   geom_smooth(method = "lm")+
  facet_wrap(~sig_list)

cor(eval_df_8$expected_list, eval_df_8$actual_list)

cor(eval_df_8$fitsig_list, eval_df_8$actual_list)
```
...
for (i in 1:Biliary_cancer_samples_size){
  ...
  method_test <- estimate_coeff(sample_prof, sigs = s_list, tissue_type = "BRCA", scale = "exome", quantiles = c("5%", "95%"), mb_size = 50)
  
  if(length(s_list) == 1){
   cosmic_1sig <- as.matrix(cosmic.signatures[,s_list])
    colnames(cosmic_8sig) <- s_list
...
Biliary_cancer_sample_names <- unique(evaluation_ds$sample_list)
mse_df_8 <- cbind(Biliary_cancer_sample_names, mse_list_EF, mse_list_EA)
mse_df_8 <- as_tibble(mse_df_8)
mse_df_8 <- mse_df_8 %>%
  mutate(
...
```{r}
write.csv(mse_df_8, "Data/mse_ds_Biliary_cancer.csv", row.names = FALSE)
```

Plotting MSE between Expected vs. fit_to_signature vs. Actual
```{r}
mse_df_8 <- read_csv("~/Desktop/curi2022/Data/mse_ds_Biliary_cancer.csv")
```
```{r}
mean(mse_df_8$mse_list_EF)
mean(mse_df_8$mse_list_EA)

histogram(mse_df_8$mse_list_EF)
histogram(mse_df_8$mse_list_EA)

mse_df_8 %>%
  ggplot(aes(x = seq(1, dim(mse_df_8)[1]), y = mse_list_EA)) +
    geom_point() +
    labs(x = "samples", y = "MSE", title = "MSE between Expected and Actual value for Biliary cancer") 

mse_df_8 %>%
  ggplot(aes(x = seq(1, dim(mse_df_8)[1]), y = mse_list_EF)) +
    geom_point() +
    labs(x = "samples", y = "MSE", title = "MSE between Expected and fit_to_signature value for Biliary cancer") 
```
