...
plot_sample_prof <- function(sample_profile){
  sample_profile <- sample_profile[,1]
  sample_profile <- as.matrix(sample_profile)
  colnames(sample_profile) <- c("prob")
...
estimate_coeff <- function(sample_profile, sigs, tissue_type, scale, quantiles, mb_size){
  ...
  #parameters testing
  #sample_profile <- generate_sample_prof(c("SBS1","SBS10c","SBS6"),c(100,300,600))
  sums <- colSums(sample_profile)
  total_mut <- unname(sums["mut"])
  sample_profile <- as.matrix(sample_profile[,1])
  colnames(sample_profile) <- c("prob")
  
  #sigs <- c("SBS1","SBS10c","SBS6")
  #tissue_type <- "BRCA"
  #scale <- "exome"
  #quantiles <- c("5%","95%")
  #mb_size <- 50
...
plot_sample_prof(sample_profile)
estimate_coeff(sample_profile, sigs = c("SBS1","SBS10b","SBS6"), tissue_type = "BRCA", scale = "exome", quantiles = c("5%", "95%"), mb_size = 50)
