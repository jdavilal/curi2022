...
generate_sample_prof <- function(sigs, nums){
  ...
  prob <- rowSums(prob)
  mut <- rowSums(df_a[,seq(1, length(sigs)*2-1, 2)])

  sample_profile <- as.matrix(cbind(mut, prob))
  sample_profile
}
```
...
## Comparing the method with fit_to_signature
```{r}
...
ColoRect exome (It takes about 10 mins)
```{r}
...
Plot for MSE between fit_to_signature and our method
```{r}
samples <- seq(1, coloE_size)
eval_df <- data_frame(samples, mse_list)
eval_df %>%
  ggplot(aes(x = samples, y = mse_list)) +
    geom_point() +
    labs(x = "samples", y = "MSE", title = "MSE between fit_to_signature and our method")
```


one exome example
```{r}
sample_prof <- generate_sample_prof(c("SBS1","SBS5","SBS40"),c(50,20,45))
method_test <- estimate_coeff(sample_prof, sigs = c("SBS1","SBS5","SBS40"), tissue_type = "COAD", scale = "exome", quantiles = c("5%", "95%"), mb_size = 50)

#Test with simulated sample profile
#fit_to_signatures(sample_prof, signatures)
fit_test <- fit_to_signatures_strict(sample_prof, cosmic.signatures[,c("SBS1","SBS5","SBS40")], max_delta = 0.002)
fit_test <- fit_test$fit_res$contribution

cbind(fit_test, method_test)
```
