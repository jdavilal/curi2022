...
eval_df <- cbind(sample_names, sig_list, expected_list, fitsig_list, actual_list)
...
ggplot(data = eval_df, mapping = aes(x = expected_list, y = actual_list))+
...
coloE_sample_names <- COSMICsample_ds %>%
  filter(Cancer.Types == "ColoRect-AdenoCA") %>%
  filter(Scale == "exome") %>%
  select(4)
coloE_sample_names <- as.matrix(coloE_sample_names)
