...
#method1
ggplot(p3_final, aes(x = sig10, y = p3)) +
  geom_point(alpha = 0.3) + 
  geom_text(data = subset(p3_final, p3 > 0.12 & sig10 < 0.1), aes(label=ext.context), vjust = 2) +
  geom_text(data = subset(p3_final, p3 > 0.14 & sig10 > 0.2), aes(label=ext.context), vjust = 2) +
  geom_text(data = subset(p3_final, sig10 > 0.3), aes(label=ext.context), vjust = 2) +
...
ggplot(data = eval_df, aes(x = expected_list, y = actual_list, label = sig_list))+
  geom_point(alpha=0.3) +
  geom_text(data = subset(eval_df, expected_list > 0.79 & actual_list < 0.6), aes(label=sig_list), hjust= -0.1, vjust= 1) +
