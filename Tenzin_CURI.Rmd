...
load_old_mutational_matrix<- 
  function(filename,
          mut.order="~/Desktop/curi2022/mutational_sig_oec/supplied_data/mut_sig.order.csv") {
...
load_cosmic_matrix<- function(mut.order="~/Desktop/curi2022/mutational_sig_oec/supplied_data/mut_sig.order.csv") {
...
signatures <- read_csv(
    "~/Desktop/curi2022/mutational_sig_oec/supplied_data/mutational.sig.csv")

# Load sequencing/cohort info for samples, filter with at
# at least 5 million Mb covered
sample.info <-
    read_csv("~/Desktop/curi2022/mutational_sig_oec/supplied_data/sample.info.csv") %>%
    filter (area.cov > 5e6)
```
...
data <- read_csv("~/Desktop/curi2022/Data/P3_sig.csv")
...
```{r}
function_test <- function(x,y,z){
  sig <-cosmic.signatures[,c(y,z)]
  sig.labels <- c("y", "z")
  colnames(sig) <- sig.labels
 
  data<- cbind(x,sig)
  data<-as_tibble(data)
  model<- lm(x ~0+ y + z, data=data)
  summary(model)
}

function_test(data$p3,10,6)
```
...
generate_sample_prof_new <- function(A, B, n1, n2){
...
data 
x <- model.matrix(p3 ~ ., data)[, -1]
y <- log(data$sig10)
glmnet(x,y ,lower.limits = 0, intercept = FALSE)
summary(model1)
