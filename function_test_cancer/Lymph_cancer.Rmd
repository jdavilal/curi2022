...
#Testing for Lymph cancer, exome data
```{r}
...
Lymph_cancer_samples <- COSMICsample_ds %>%
  filter(Cancer.Types == "Lymph-BNHL") %>%
  filter(Scale == "exome")

Lymph_cancer_samples_withnames <- Lymph_cancer_samples %>%
  select(4,6:64) 
Lymph_cancer_samples <- Lymph_cancer_samples %>%
  select(6:64) 
Lymph_cancer_samples_size <- dim(Lymph_cancer_samples)[1]

sample_names_list <- Lymph_cancer_samples_withnames %>%
    select(1)
sample_names_list <- as.matrix(sample_names_list)
```


Making the final Dataframe for Lymph cancer 
...
for (i in 1:23){
  ...
  mut <- as.numeric(Lymph_cancer_samples[i,])
  nonzero_sig <- (which(mut %nin% c(0, NA)))
  list <- colnames(Lymph_cancer_samples)
  ...
  method_test <- estimate_coeff(sample_prof, sigs = s_list, tissue_type = "DLBC", scale = "exome", quantiles = c("5%", "95%"), mb_size = 50)
...
eval_df_7 <- cbind(sample_list, sig_list, expected_list, fitsig_list, actual_list)
eval_df_7 <- as_tibble(eval_df_7)
eval_df_7 <- eval_df_7 %>%
  mutate(
...
eval_df_7 <- cbind(sample_names, sig_list, expected_list, fitsig_list, actual_list)
eval_df_7 <- as_tibble(eval_df_7)
``` 

```{r}
write.csv(eval_df_7, "Data/Lymph_cancer_evaluation_ds.csv", row.names = FALSE)
```

Plot the Actual vs. Expected
```{r}
eval_df_7 <- read_csv("~/Desktop/curi2022/Data/Lymph_cancer_evaluation_ds.csv")
ggplot(data = eval_df_7, mapping = aes(x = expected_list, y = actual_list))+
   geom_point(alpha=0.3)+
   geom_smooth(method = "lm")+
  facet_wrap(~sig_list)

cor(eval_df_7$expected_list, eval_df_7$actual_list)

cor(eval_df_7$actual_list, eval_df_7$fitsig_list)
```
...
for (i in 1:Lymph_cancer_samples_size){
...
Lymph_cancer_sample_names <- unique(evaluation_ds$sample_list)
mse_df_7 <- cbind(Lymph_cancer_sample_names, mse_list_EF, mse_list_EA)
mse_df_7 <- as_tibble(mse_df_7)
mse_df_7 <- mse_df_7 %>%
  mutate(
...
```{r}
write.csv(mse_df_7, "Data/mse_ds_Lymph_cancer.csv", row.names = FALSE)
```

Plotting MSE between Expected vs. fit_to_signature vs. Actual
```{r}
mse_df_7 <- read_csv("~/Desktop/curi2022/Data/mse_ds_Lymph_cancer.csv")
```
```{r}
mean(mse_df_7$mse_list_EF)
mean(mse_df_7$mse_list_EA)

histogram(mse_df_7$mse_list_EF)
histogram(mse_df_7$mse_list_EA)

mse_df_7 %>%
  ggplot(aes(x = seq(1, dim(mse_df_7)[1]), y = mse_list_EA)) +
    geom_point() +
    labs(x = "samples", y = "MSE", title = "MSE between Expected and Actual value for Lymph cancer") 

mse_df_7 %>%
  ggplot(aes(x = seq(1, dim(mse_df_7)[1]), y = mse_list_EF)) +
    geom_point() +
    labs(x = "samples", y = "MSE", title = "MSE between Expected and fit_to_signature value for Lymph cancer") 
```
