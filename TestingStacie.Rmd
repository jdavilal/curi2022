...
#plotting new data and doing eda
```{r}
new_comparison <- df %>%
  mutate(method_coeff = as.numeric(actual_list),
         true_coeff =as.numeric(expected_list))

ggplot(data = new_comparison, mapping = aes(x = true_coeff, y = method_coeff)) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "lm")


histogram(new_comparison$true_coeff - new_comparison$method_coeff)

cor(new_comparison$true_coeff, new_comparison$method_coeff) #0.2241283

test <- new_comparison %>%
  mutate(difference = (method_coeff - true_coeff))
...
#Reading in correctly named dataset 
```{r}
comparison_ds <- read_csv("Data/comparison_ds.csv")
View(comparison_ds)
```

#Trying to figure out what is going on with the corners
```{r}
ggplot(data = comparison_ds, mapping = aes(x = expected_list, y = actual_list))+
   geom_point(alpha=0.3)+
   geom_smooth()

test <- comparison_ds %>%
  filter(abs((actual_list - expected_list)) > 0.8)

weirdsamples <- unique(test$sample_names)

weirdsample_df <- coloE_samples %>%
  filter(Sample.Names %in% weirdsamples)


test2 <- comparison_ds %>%
  filter(abs((actual_list - expected_list)) > 0.5)

weirdsamples2 <- unique(test2$sample_names)

weirdsample_df2 <- coloE_samples %>%
  filter(Sample.Names %in% weirdsamples2)
```

```{r}
mut <- as.numeric(weirdsample_df[1,])
  nonzero_sig <- (which(mut %nin% c(0, NA)))-1
  weirdsample_df_samples_noname <- weirdsample_df %>%
    select(2:60)
  list <- colnames(weirdsample_df_samples_noname)
  s_list <- list[nonzero_sig]
  s_list <- str_remove(s_list, ".Mutations")
  m_list <- mut[mut %nin% c(0, NA)]
